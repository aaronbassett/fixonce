...
export function registerExpandTool(server: McpServer): void {
  server.tool(
    ...
    {
      cache_key: z.string().min(1).describe("Cache key from the overflow section of a fixonce_query result"),
      verbosity: z.enum(["small", "medium", "large"]).optional().describe("Detail level of returned memory (default: 'small')"),
    },
    async (args) => {
      try {
