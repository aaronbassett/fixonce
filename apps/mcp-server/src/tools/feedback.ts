  ...
  server.tool(
    ...
    "Submit feedback on a memory to help improve quality over time. Feedback tags indicate whether a memory was helpful, accurate, outdated, or damaging. Use 'suggested_action' to flag a memory for removal or correction. Memories flagged with 'remove', 'fix', or 'damaging' are surfaced for human review.",
    {
      memory_id: z
        .string()
        .uuid()
        .describe("UUID of the memory to provide feedback on"),
      text: z
        .string()
        .nullable()
        .optional()
        .describe(
          "Free-form feedback text explaining the issue or observation",
        ),
      tags: z
        .array(
          z.enum([
      ...
      suggested_action: z
        .enum(["keep", "remove", "fix"])
        .nullable()
        .optional()
        .describe(
          "Suggested action: 'keep' if fine, 'remove' if wrong, 'fix' if needs correction",
        ),
    },
    async (args) => {
      try {
