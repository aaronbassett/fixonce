...
mod tests {
    ...
    fn load_valid_token_fails_when_no_token_stored() {
        // In a clean test environment there is no stored token.
    ...
    fn expired_token_is_rejected_by_token_manager() {
        // Verify the expiry check that load_valid_token relies on.
        let mgr = TokenManager::new();
        let expired = fake_jwt(Some(1)); // exp = 1 second past epoch
        assert!(
    ...
    fn valid_token_passes_expiry_check() {
        let mgr = TokenManager::new();
        let valid = fake_jwt(Some(u64::MAX / 2)); // far future
        assert!(
    ...
    /// Build a minimal JWT with the given `exp` claim (no signature verification).
