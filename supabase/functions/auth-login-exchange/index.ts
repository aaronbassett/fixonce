...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  if (!tokenResponse.ok) {
    ...
    console.error("auth-login-exchange: token exchange error", tokenResponse.status, body);
    return errorResponse(
      502,
      "EXCHANGE_REJECTED",
  ...
  if (!accessToken) {
    console.error("auth-login-exchange: no access_token in response, keys:", Object.keys(tokenPayload ?? {}));
    return errorResponse(
      502,
      "MISSING_TOKEN",
